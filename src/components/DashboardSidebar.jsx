...
// import logo from '/public/Assets/jsc_logo_hitam.png'
...
const MenuItem = ({ path, icon, label, isActive, onClick }) => (
  <div
    className={`${isActive ? "rounded-sm border-e-[5px] border-[#003835] " : ""}`}
  >
    <div className="flex items-center justify-center w-full h-full font-semibold teksSide text-[#007471]">
      <p
        className={`w-2/3 font-semibold px-2 py-1 items-center rounded-md flex justify-between cursor-pointer ${isActive ? "bg-linear-to-l from-[#003835] to-[#007471] text-white" : ""}`}
        onClick={onClick}
      >
        {label} {icon}
      </p>
      ...
      <div className="parent utama w-full h-full flex flex-col j items-center">
        <div className="side bag atas w-full h-4/6 flex flex-col items-center">
          <div className="logo flex justify-center w-full mb-5">
            <img
              className="md:w-[55%] lg:w-[40%]"
              src="/Assets/jsc_logo_hitam.png"
            />
          </div>
